  ...
  if (!preview) {
    return <main className="onboarding-page" onPaste={homeStep === 2 ? pasteGridScreenshot : undefined}>
      {homeStep === 1 ? <section className="onboarding-screen onboarding-identity" aria-labelledby="identity-title">
        ...
        <div className="onboarding-bottom"><label className="onboarding-upload"><CaptureIcon /><span><b>캡처한 이미지 선택</b><small>프로필 캡처 한 장이면 충분해요.</small></span><strong>↑</strong><input type="file" accept="image/jpeg,image/png,image/webp" onChange={chooseGridScreenshot} /></label><button className="onboarding-primary" type="button" onClick={startEmptyProfile}><b>빈 피드로 바로 시작</b></button><div className="onboarding-secondary-actions"><button className="onboarding-text-button" type="button" onClick={startEmptyProfile}>건너뛰기</button></div></div>
      </section>}
    </main>;
  }
